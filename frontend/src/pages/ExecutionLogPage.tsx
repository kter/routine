    ...
    return (
      <div className="mx-auto max-w-2xl space-y-4 animate-fade-up">
        ...
        <div className="space-y-2">
          {[...Array(3)].map((_, i) => <div key={i} className="h-16 rounded-md shimmer" />)}
        </div>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-2xl space-y-5 animate-fade-up">
      ...
      <div
        ...
        style={{
          ...
          borderLeft: execution.status === "completed"
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <p className="font-medium" style={{ color: "hsl(210 20% 88%)" }}>{execution.taskTitle}</p>
          <StatusBadge status={execution.status} />
        </div>

        <div className="flex flex-wrap gap-x-5 gap-y-1">
          <span className="font-mono-data text-[11px]" style={{ color: "hsl(215 16% 40%)" }}>
        ...
        {execution.notes && (
          <p className="text-sm leading-relaxed" style={{ color: "hsl(215 16% 48%)" }}>
            {execution.notes}
          </p>
        )}
      </div>
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="mt-0.5 shrink-0">
                    {isCompleted ? (
                      <CheckCircle2 className="h-4 w-4" style={{ color: "hsl(160 60% 45%)" }} />
                    ) : isSkipped ? (
                      <MinusCircle className="h-4 w-4" style={{ color: "hsl(32 95% 50%)" }} />
                    ) : (
                      <Circle className="h-4 w-4" style={{ color: "hsl(215 16% 30%)" }} />
                    )}
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      <p className="text-sm font-medium leading-snug" style={{ color: isCompleted ? "hsl(210 20% 82%)" : "hsl(215 16% 50%)" }}>
                    ...
                    {step.evidenceText && (
                      <div className="mt-2">
                        <p className="font-mono-data text-[10px] uppercase tracking-widest mb-1" style={{ color: "hsl(215 16% 34%)" }}>
                          Evidence
                        </p>
                        <p className="text-xs whitespace-pre-wrap leading-relaxed" style={{ color: "hsl(215 16% 52%)" }}>
                          {step.evidenceText}
                        </p>
                      </div>
                    )}
                    ...
                    {step.completedAt && (
                      <p className="mt-1.5 font-mono-data text-[10px]" style={{ color: "hsl(215 16% 32%)" }}>
